...
const Button = ({ label, link, handleClick, disabled, outline, small, icon: Icon, iconSize } : ButtonProps) => {
  return (
    <button
        onClick={handleClick}
        disabled={disabled}
        className={`relative disabled:opacity-70 disabled:cursos-not-allowed hover:opacity-80 transition w-full md:w-auto h-max	cursor-pointer flex justify-between items-center gap-2
